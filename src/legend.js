...
// -- the colored squares indicating happiness group -- //
export function LegendBase(props) {
    const {x, y, colormap, happinessLevels, hoveredLegend, setHoveredLegend} = props;
        const w = 15;
        const h = 20;
        ...
        // -- each happinessLevels returned below will be a rectangle filled with color -- //
        ...
        return <g transform = {`translate(${x}, ${y})`}>
            {happinessLevels.map( (d, idx) => {
                return <rect key={idx+"legend"} x={0} y={h*(happinessLevels.length-1)-idx*h} width={w} height={h} 
                style={{fill:colormap(d)}} opacity={opacity(d)} 
                onMouseEnter={() => setHoveredLegend(d)} onMouseOut={() => setHoveredLegend(null)}/>
            } )}
            {happinessLevels.map((d, idx) => {
                return <text key={idx+"legendText"} x={20} y={h*(happinessLevels.length-1)-idx*h+15} opacity={opacity(d)}
                onMouseEnter={() => setHoveredLegend(d)} onMouseOut={() => setHoveredLegend(null)}>
                    {d}
                </text>
            } )}
            ...
            <g key={"unknow_rect"} >
                <rect x={0} y={h*happinessLevels.length} width={w} height={h} 
                ...
                <text x={20} y={happinessLevels.length*h+15} opacity={hoveredLegend && "6. Unknown" !== hoveredLegend ? 0.5 : 1}
